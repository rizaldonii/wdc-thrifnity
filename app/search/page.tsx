...
const allLocations = Array.from(
  new Set(
    tailors.map(
      (tailor) => `${tailor.location.city}, ${tailor.location.province}`
    )
  )
);
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    if (selectedSpecialties.length > 0) {
      result = result.filter((tailor) =>
        selectedSpecialties.some((specialty) =>
          tailor.specialty.includes(specialty)
        )
      );
    }
    ...
    if (selectedTags.length > 0) {
      result = result.filter((tailor) =>
        selectedTags.some((tag) => tailor.tags.includes(tag))
      );
    }
    ...
    if (selectedPriceRanges.length > 0) {
      result = result.filter((tailor) => {
        ...
        const avgPrice =
          tailor.services.reduce((sum, service) => sum + service.price, 0) /
    ...
    switch (sortBy) {
      ...
      case "price_low":
        result.sort((a, b) => {
          const avgPriceA =
            a.services.reduce((sum, service) => sum + service.price, 0) /
            a.services.length;
          const avgPriceB =
            b.services.reduce((sum, service) => sum + service.price, 0) /
            b.services.length;
    ...
    if (selectedSpecialties.length === 1)
      params.set("specialty", selectedSpecialties[0]);
  ...
  const removeFilter = (type: string, value?: string) => {
    switch (type) {
      ...
      case "price":
        if (value !== undefined) {
          setSelectedPriceRanges((prev) =>
            prev.filter((i) => i !== Number.parseInt(value))
          );
        }
        break;
      case "rating":
...
function DesktopFilters({
  ...
  return (
    <div className="space-y-6">
      <Accordion
        type="multiple"
        defaultValue={["specialties", "tags", "location", "price", "rating"]}
      >
